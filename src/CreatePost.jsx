...
export default function CreatePost({ setSubreddit, setTitle, setDesc, createPost }) {
  ...
  return (
    <div className='w-full max-w-md h-fit bg-white border-gray-100 border rounded-lg'> 
      ...
      <form onSubmit={handleCreatePost} className='m-3 flex flex-col gap-2 items-center'>
        <input name='subreddit' onChange={e => setSubreddit(e.target.value)} type="text" placeholder='subreddit' className='w-full border rounded-sm px-2 text-black' required/>
        <input name='title' onChange={e => setTitle(e.target.value)} type="text" placeholder='title' className='w-full border rounded-sm px-2 text-black' required/>
        <textarea name="description" onChange={e => setDesc(e.target.value)} placeholder='description' className='w-full border rounded-sm p-2 text-black resize-none' required></textarea>
